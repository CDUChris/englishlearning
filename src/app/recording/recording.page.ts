...
export class RecordingPage extends PageBase implements OnInit {
  ...
  async fetchConversationDetail() {
    ...
    const mp3Ref = ref(storage, 'conversations/' + this.activatedRoute.snapshot.paramMap.get('title') + '/' + this.activatedRoute.snapshot.paramMap.get('title') + '.mp3');
    const wordRef = ref(storage, 'conversations/' + this.activatedRoute.snapshot.paramMap.get('title') + '/' + this.activatedRoute.snapshot.paramMap.get('title') + '.txt');
    try {
      const [mp3Url, wordsUrl] = await Promise.all([getDownloadURL(mp3Ref), getDownloadURL(wordRef)]);;
  ...
  async compare(text) {
    const compareResult = similar(text, this.conversation.text.filter((cur, index) => index % 2).join(''), null);
